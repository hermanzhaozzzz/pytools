...
class DLWorker:
    def __init__(self, name:str, url:str, range_start, range_end, cache_dir, finish_callback):
        self.name = name
        self.url = url
        self.cache_filename = os.path.join(cache_dir, name + ".dl")
...
class Downloader:
    def __init__(self, url:str, download_dir:str=f".{os.sep}__down{os.sep}", blocks_num:int=8, debug=True, file_name=None):
        """多线程下载器
        url: str, 要下载的资源路径
        download_dir: str, 指定要下载到的位置，默认为./__down/
        blocks_num: int, 指定线程数，默认8个线程
        debug: bool, 是否显示日志，默认显示日志
        file_name: 默认使用url中的文件名作为文件名，可以单独指定文件名
        作者: 赵华男
        邮箱: hermanzhaozzzz@gmail.com
        """
        ...
        if not self.__bad_url_flag:
            ...
            # 建立缓存目录
            self.cache_dir = f".{os.sep}__down{os.sep}.cache{os.sep}"
            ...
            if self.debug:
                sys.stdout.write(f"----- Downloader [v1.0] -----\n[url] {self.url}\n[path] {pathfilename}\n[size] {readable_size}\n")
    ...
    def __get_cache_filenames(self):
        return glob.glob(f"{self.cache_dir}{self.filename}.*.dl")

    def __get_ranges_from_cache(self):
        # 形如 ./cache/filename.1120.dl
        ranges = []
    ...
    def __sew(self):
        ...
        with open(f"{os.path.join(self.download_dir, self.filename)}", "wb") as f:
            for start, _ in self.__get_ranges_from_cache():
                cache_filename = f"{self.cache_dir}{self.filename}.{start}.dl"
